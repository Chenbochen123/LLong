...
{
    public class PressTempSource : TableSyn.Source
    {
        /// <summary>
        /// 查询数据
        /// </summary>
        ...
        public override DataTable SelectData(DbSynResult dbSyn)
        {
            ...
            //获取该数据库中温压曲线表
            dbHelper.CommandText = @"SELECT Name FROM SysObjects Where XType='U' and name like '%tb_PP_PressTemp%'
 and name not like '%'+CONVERT(varchar(10),GETDATE(),112)+'%' ORDER BY Name desc";
            DataTable baseTable = dbHelper.ToDataTable();
            for (int i = 0; i < baseTable.Rows.Count; i++)
            {
                dbHelper.CommandText = string.Format("select * from {0} where UpFlag=0", baseTable.Rows[i]["Name"].ToString());
                if (dbHelper.ToDataTable().Rows.Count > 0)
                {
                    #region 整理 sql 语句
                    table.TableName = baseTable.Rows[i]["Name"].ToString();
