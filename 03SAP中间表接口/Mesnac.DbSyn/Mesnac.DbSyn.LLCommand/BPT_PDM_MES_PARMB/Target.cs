...
namespace Mesnac.DbSyn.LLERP.BPT_PDM_MES_PARMB
{
    /// <summary>
    /// 目标 命令
    /// </summary>
    public class Target : TableSyn.Target
    {
        ...
        protected override int getDefaultTargetData(DbSynResult dbSyn, KVSQL kvsql, bool isUpdate)
        {
                ...
                && (!string.IsNullOrWhiteSpace(parmTypeCode.ToString())))
            {
                ...
                sqlsb.Append("select objid from BPT_MOLDING_TECH_PARM_TYPE where 1=1");
                ...
                {
                    sqlsb = new StringBuilder();
                    sqlsb.Append("select SEQ_BPT_MOLDING_TECH_PARM_TYPE.nextval from dual");
                    thisDbHelper.CommandText = sqlsb.ToString();
                    parmTypeId = Convert.ToInt32(thisDbHelper.ToScalar().ToString());

                    sqlsb = new StringBuilder();
                    sqlsb.Append(@"insert into BPT_MOLDING_TECH_PARM_TYPE
                          (OBJID,RECORD_USER_ID,RECORD_TIME,BACKUP_FLAG,DELETE_FLAG,
                ...
                && (!string.IsNullOrWhiteSpace(parmInfoCode.ToString())))
            {
                ...
                sqlsb.Append("select objid from BPT_MOLDING_TECH_PARM_INFO where 1=1");
                ...
                {
                    sqlsb = new StringBuilder();
                    sqlsb.Append("select SEQ_BPT_MOLDING_TECH_PARM_INFO.nextval from dual");
                    thisDbHelper.CommandText = sqlsb.ToString();
                    parmInfoId = Convert.ToInt32(thisDbHelper.ToScalar().ToString());

                    sqlsb = new StringBuilder();
                    sqlsb.Append(@"insert into BPT_MOLDING_TECH_PARM_INFO
                          (OBJID,RECORD_USER_ID,RECORD_TIME,BACKUP_FLAG,DELETE_FLAG,
