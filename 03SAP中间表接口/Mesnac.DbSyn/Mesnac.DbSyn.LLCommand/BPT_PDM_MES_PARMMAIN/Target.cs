...
namespace Mesnac.DbSyn.LLERP.BPT_PDM_MES_PARMMAIN
{
    /// <summary>
    /// 目标 命令
    /// </summary>
    public class Target : TableSyn.Target
    {
        ...
        protected override int getDefaultTargetData(DbSynResult dbSyn, KVSQL kvsql, bool isUpdate)
        {
            ...
            #region EFFECT_DATE
            {
                kvsql.Add("EFFECT_DATE", DateTime.Now);
            }
            #endregion
            ...
            {
                var value = 0;
                var sap_code = getERPMaterialId(row, "SAPMAT");
                if (!string.IsNullOrWhiteSpace(sap_code.ToString()))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        sqlsb.Append("UPDATE SBM_MATERIAL SET PDM_CODE='").Append(getRowString(row, "PDMMAT")).Append("' WHERE 1=1");
                        thisKVSql.Add("SAP_CODE", sap_code);
                        thisKVSql.ToWhere(thisDbHelper, sqlsb);
                        thisDbHelper.CommandText = sqlsb.ToString();
            ...
            #region TECH_TYPE
            {
                    ...
                    && (!string.IsNullOrWhiteSpace(stepName.ToString())))
                {
                    ...
                    sqlsb.Append("select objid from BPT_MOLDING_TECH_TYPE where 1=1");
                    ...
                    {
                        sqlsb = new StringBuilder();
                        sqlsb.Append("select SEQ_BPT_MOLDING_TECH_TYPE.nextval from dual");
                        thisDbHelper.CommandText = sqlsb.ToString();
                        value = Convert.ToInt32(thisDbHelper.ToScalar().ToString());

                        sqlsb = new StringBuilder();
                        sqlsb.Append(@"insert into BPT_MOLDING_TECH_TYPE
            ...
            var parmGroupName =getRowString( row,"PARMINDXC");
            if (string.IsNullOrWhiteSpace(parmGroupName))
            {
                parmGroupName = getRowString(row, "PARMINDXB");
            }
            ...
            if (!string.IsNullOrWhiteSpace(parmGroupName))
            {
                ...
                sqlsb.Append("select objid from BPT_MOLDING_TECH_PARM_GROUP where 1=1");
                ...
                {
                    sqlsb = new StringBuilder();
                    sqlsb.Append("select SEQ_BPT_MOLDING_TECH_PARM_GP.nextval from dual");
                    thisDbHelper.CommandText = sqlsb.ToString();
                    parmGroupId = Convert.ToInt32(thisDbHelper.ToScalar().ToString());

                    sqlsb = new StringBuilder();
                    sqlsb.Append(@"insert into BPT_MOLDING_TECH_PARM_GROUP
            ...
            if (!kvsql.ContainsKey("MATERIAL_ID"))
            {
                loginfo.Append("无子物料 MATERIAL_ID(SAPMAT)=").Append(getERPMaterialId(row, "SAPMAT"));
            }
