...
namespace Mesnac.DbSyn.LLERP.CPT_PDM_MES_PARMMAIN
{
    /// <summary>
    /// 目标 命令
    /// </summary>
        ...
        protected override int getDefaultTargetData(DbSynResult dbSyn, KVSQL kvsql, bool isUpdate)
        {
            ...
            #region TECH_TYPE
            {
                    ...
                    && (!string.IsNullOrWhiteSpace(stepName.ToString())))
                {
                    ...
                    sqlsb.Append("select objid from CPT_CURING_TECH_TYPE where 1=1");
                    ...
                    {
                        sqlsb = new StringBuilder();
                        sqlsb.Append("select SEQ_CPT_CURING_TECH_TYPE.nextval from dual");
                        thisDbHelper.CommandText = sqlsb.ToString();
                        value = Convert.ToInt32(thisDbHelper.ToScalar().ToString());

                        sqlsb = new StringBuilder();
                        sqlsb.Append(@"insert into CPT_CURING_TECH_TYPE
