...
var McUI_SearchBox_SearchBoxVCbmMaterial_Request = function (record) {
    var materialFlag = App.ui_s_MaterialFlag.getValue();
    if (materialFlag == '1') {
        ...
        App.ui_s_LAST_MATERIAL_ID_TriggerField.setValue(record.data.MATERIAL_NAME);
        App.ui_s_LAST_MATERIAL_ID.setValue(record.data.OBJID);
    }
}

var get_MATERIAL_ID_TriggerField = function (field, trigger, index) {//成品胎规格查询
    if (index == 1) {
        App.ui_s_MaterialFlag.setValue('1');
...
var get_LAST_MATERIAL_ID_TriggerField = function (field, trigger, index) {//成品胎规格查询
    if (index == 1) {
        App.ui_s_MaterialFlag.setValue('2');
        App.McUI_SearchBox_SearchBoxVCbmMaterial_Window.show();
    }
    else if (index == 0) {
        this.clear();
        App.ui_s_LAST_MATERIAL_ID.clear();
    }
}
