...
var afterShow_winUpdate = function () {
    var planDetailName = App.ui_u_PLAN_DATE.getRawValue()
        ...
        + "," + App.ui_u_PLAN_EQUIP_NAME.getRawValue()
    ...
    var shiftMasterName = App.ui_u_SHIFT_DATE.getRawValue()
        + "," + App.ui_u_SHIFT_NAME.getRawValue()
        + "," + App.ui_u_CLASS_NAME.getRawValue()
...
//-------接班信息-----查询带回弹出框--BEGIN
var McUI_SearchBox_SearchBoxBpmMoldingShiftMaster_Request = function (record) {
    if (!App.winAdd.hidden) {
        ...
        App.ui_i_SHIFT_NAME.setValue(record.data.SHIFT_NAME);
        App.ui_i_CLASS_NAME.setValue(record.data.CLASS_NAME);
        ...
        App.ui_u_SHIFT_NAME.setValue(record.data.SHIFT_NAME);
        App.ui_u_CLASS_NAME.setValue(record.data.CLASS_NAME);
...
var get_SHIFT_MASTER_ID_TriggerField = function (field, trigger, index) {//接班查询
    App.McUI_SearchBox_SearchBoxBpmMoldingShiftMaster_Window.show();
}

Ext.create("Ext.window.Window", {//接班查询带回窗体
    id: "McUI_SearchBox_SearchBoxBpmMoldingShiftMaster_Window",
    height: 460,
    hidden: true,
    width: 560,
    html: "<iframe src='../../McUI/SearchBox/SearchBoxBpmMoldingShiftMaster.aspx' width=100% height=100% scrolling=no  frameborder=0></iframe>",
...
//-------成型计划信息-----查询带回弹出框--BEGIN
var McUI_SearchBox_SearchBoxBppMoldingPlanDetail_Request = function (record) {
    if (!App.winAdd.hidden) {
        ...
        App.ui_i_PLAN_EQUIP_NAME.setValue(record.data.EQUIP_NAME);
        App.ui_i_MATERIAL_NAME.setValue(record.data.MATERIAL_NAME);

        App.ui_i_PLAN_DATE.setValue(record.data.PLAN_DATE);
        App.ui_i_PLAN_SHIFT_NAME.setValue(record.data.SHIFT_NAME);
    }
    else {
        ...
        App.ui_u_PLAN_EQUIP_NAME.setValue(record.data.EQUIP_NAME);
        App.ui_u_MATERIAL_NAME.setValue(record.data.MATERIAL_NAME);

        App.ui_u_PLAN_DATE.setValue(record.data.PLAN_DATE);
        App.ui_u_PLAN_SHIFT_NAME.setValue(record.data.SHIFT_NAME);
    }
}

var get_PLAN_DETAIL_ID_TriggerField = function (field, trigger, index) {//接班查询
    App.McUI_SearchBox_SearchBoxBppMoldingPlanDetail_Window.show();
}

Ext.create("Ext.window.Window", {//成型计划查询带回窗体
    id: "McUI_SearchBox_SearchBoxBppMoldingPlanDetail_Window",
    height: 460,
    hidden: true,
    width: 560,
    html: "<iframe src='../../McUI/SearchBox/SearchBoxBppMoldingPlanDetail.aspx' width=100% height=100% scrolling=no  frameborder=0></iframe>",
    bodyStyle: "background-color: #fff;",
    closable: true,
    title: "请选择成型计划",
    modal: true
